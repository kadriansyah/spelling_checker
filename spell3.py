...
class SpellCorrector:
    ...
    def validate(self, sentence, debug=False):
        ...
        words =  [x.strip().lower() for x in sentence.split() if x.strip()] # Hapus seluruh empty char pada list
        # print(words)
        ...
        for idx, word in enumerate(words):
            ...
            if tag:
                word = tag.group(2)
                if word in self.words:
                    valid.append(tag.group(1) + word.lower())
                else:
                    candidates = self.candidates(word.lower())
                    if  idx == 0 :
                        max_word = max([w for w in candidates], key=lambda word: self.model.unigram_prob(word))
                        valid.append(tag.group(1) + max_word)
                        if debug:
                            print('candidates for '+ word +': '+ str(candidates) +', max prob word is '+ max_word.lower())
            ...
            if tag:
                word = tag.group(1)
                if word in self.words:
                    valid.append(word.lower() + tag.group(2))
                else:
                    candidates = self.candidates(word.lower())
                    if  idx == 0 :
                        max_word = max([w for w in candidates], key=lambda word: self.model.unigram_prob(word))
                        valid.append(max_word + tag.group(2))
                        if debug:
                            print('candidates for '+ word +': '+ str(candidates) +', max prob word is '+ max_word.lower())
